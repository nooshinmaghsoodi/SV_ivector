function [N, F, viterbi_log_likelihood, segmentationInfo] = compute_hmm_bw_stats_subspace(data, transform, hmm, transformedHmm, statFilename)
...
if (length(size(sigma)) == 2)
    ...
    logprob = zeros(length(log_w), size(data, 2));
    for i = 1 : size(sigma, 3)
        sigma_inv = inv(sigma(:, :, i));
        C = mu(:, i)' * sigma_inv * mu(:, i) + log(det(sigma(:, :, i))) + ndim * log(2 * pi); %#ok<*MINV>
        D = sum(data .* (sigma_inv * data)) - 2 * mu(:, i)' * sigma_inv * data;
        logprob(i, :) = -0.5 * (D + C) + log_w(i);
    end
end
