function [hmms, statesAlignments, digitIdx] = getPart3DigitHmms(ubmModels)
transMap = getPart3FilesIndices();
...
[modelsIdx, digitIdx ]= convertTrans2ModelsIdx(transMap, map, digitMap);
keys = transMap.keys;
...
function [modelsIdx, digitIdx ] = convertTrans2ModelsIdx(transMap, map, digitMap)
modelsIdx = cell(transMap.Count, 1);
digitIdx = cell(transMap.Count, 1);
keys = transMap.keys;
for i = 1 : transMap.Count
    parts = strsplit(transMap(keys{i}));
    idx = zeros(1, length(parts));
    dIdx = zeros(1, length(parts));
    for j = 1 : length(parts)
        idx(j) = map(parts{j});
        dIdx(j) = digitMap(parts{j});
    end
    modelsIdx{str2double(keys{i})} = [map('SIL'), idx, map('SIL')];
    digitIdx{str2double(keys{i})} = [digitMap('SIL'), dIdx, digitMap('SIL')];
end
